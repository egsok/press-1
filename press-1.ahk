...
FocusWorkspace(projectName, gentle := false, editorExe := "", cycleKey := "") {
    ...
    P1_SignalFocusedWindow(choice.hwnd)
    if choice.total > 1 {
...
P1_SignalFocusedWindow(hwnd) {
    ...
    SetTimer(P1_PulseWindowOutline.Bind(hwnd), -80)
}
...
P1_PulseWindowOutline(hwnd, *) {
    ...
    thickness := Max(4, Round(5 * A_ScreenDPI / 96))
    if w < 2 * thickness || h < 2 * thickness
        return
    overlays := []
    for rect in P1_FocusOutlineRects(x, y, w, h, thickness) {
        if rect.w <= 0 || rect.h <= 0
            continue
        g := Gui("-Caption +AlwaysOnTop +ToolWindow +E0x20 +E0x08000000")
        g.BackColor := "E11B76"
        g.Show("NA x" rect.x " y" rect.y " w" rect.w " h" rect.h)
        overlays.Push(g)
    }
...
DebugPending() {
    global PENDING_DIR, PROXY_DIR, PERM_DIR
