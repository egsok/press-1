...
; Monitor identity/preferences are stable, but work-area geometry is not: the
; taskbar (or another registered desktop toolbar) can change it without a
; WM_DISPLAYCHANGE. Refresh immediately before every popup Show as the final
; consistency barrier. A transient enumeration failure keeps the last known-good
; catalog rather than turning a visible prompt into a silent no-op.
P1_RefreshDisplayCatalog(rebuildMenu := true) {
    global P1_DISPLAY_CATALOG
    fresh := []
    try fresh := P1_GetDisplayCatalog()
    if fresh.Length > 0
        P1_DISPLAY_CATALOG := fresh
    if rebuildMenu
        try P1_RebuildDisplayMenu()
    return P1_DISPLAY_CATALOG
}

P1_IsWorkAreaChange(wParam) => wParam = 0x002F  ; SPI_SETWORKAREA
...
class PromptPopup {
    ...
    static Show(prompts) {
        ...
        ; The resident may have started before Explorer/taskbars settled, or a
        ; work-area broadcast may have been missed while it was suspended.
        P1_RefreshDisplayCatalog(false)
        if !this.EnsureGdip() {
...
; Display/work-area changes can fire in bursts while Windows applies a new
; topology, DPI, taskbar position, or toolbar reservation. Debounce both paths
; before reading them and rebuilding physical-pixel popup windows.
OnMessage(0x007E, P1_OnDisplayChange)  ; WM_DISPLAYCHANGE
OnMessage(0x001A, P1_OnWorkAreaChange) ; WM_SETTINGCHANGE
...
P1_OnWorkAreaChange(wParam, *) {
    if P1_IsWorkAreaChange(wParam)
        P1_OnDisplayChange()
}
