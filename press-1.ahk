...
; window (gentle — no Alt-dance) and let the user press the digit in the TUI.
FocusCodexEditorTerminal(promptInfo) {
    editorExe := promptInfo.HasOwnProp("editor_exe") ? promptInfo.editor_exe : ""
    hwnd := FindEditorByName(promptInfo.project_name, editorExe)
    if hwnd
...
; "D:\dev\proj\" still yields "proj". Empty in → empty out (the caller drops the
...
ReadAllProxyPending() {
    ...
    Loop Files, PROXY_DIR "\*.json" {
        ...
        ; Subtitle has no separate slot on the card — fold the reason after the
        ; command (em dash), keep the bare command for the expand view (full).
        short := reason != "" ? command " " Chr(0x2014) " " reason : command
...
class PromptPopup {
    ...
    static _ReconcileBody() {
        ...
        for i, info in prompts {
            ...
            if existing {
                ...
                ; redraw can't resize the DIB). Must precede the content-diff path.
    ...
    static _RebuildCard(old, info, slot) {
        ep := this._epoch
        fresh := this._BuildCard(info, slot)      ; reads expandedKeys → new height; g.Show pumps
        oldIdx := 0
        Loop this.cards.Length
    ...
    static _DrawCard(card, pressed := "") {
        ...
        ; project name (title) — from after any pill, up to the ✕. The ACTIVE card
        ; prints in register (sharp); queued cards keep the resting magenta
        ; mis-registration ghost (NNV signature: активная = сведённая).
        if !card.active
            P1_TxtClip(GR, d.subj, titleX + misOff, ty + misOff, rightEdge - titleX, "s" fz.subj " NoWrap", co.magGhost, W,H, "", F.sansSemi)
        P1_TxtClip(GR, d.subj, titleX, ty, rightEdge - titleX, "s" fz.subj " NoWrap", co.heading, W,H, "", F.sansSemi)
