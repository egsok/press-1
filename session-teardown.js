...
//
// Stop — blanket delete of the session's permission/picker pendings. The turn
// is over, and a waiting approval prompt BLOCKS the turn, so nothing answerable
// can still exist. kind:"attention" is deliberately excluded: it is CREATED at
// Stop because Codex has ended specifically to wait for user text, and its owner
// is the next UserPromptSubmit / the rollout watcher / SessionEnd.
...
process.stdin.on("end", () => {
    ...
    if (sid) {
      ...
      for (const f of fs.readdirSync(PENDING_DIR)) {
        ...
        try {
          ...
          if (entry.session_id !== sid || entry.kind === "attention") continue;
  ...
  // Codex Stop requires JSON stdout on exit 0. `{}` is also valid/no-op for
  // Claude and PostToolUse, and prevents the "invalid stop hook JSON" failure.
