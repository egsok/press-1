...
(async () => {
  ...
  console.log("T7: non-interactive codex exec sessions never ask the user");
  clearPending();
  const sessions = path.join(PROFILE, ".codex", "sessions", "2026", "08", "11");
  fs.mkdirSync(sessions, { recursive: true });
  const execTranscript = path.join(sessions, "rollout-exec.jsonl");
  fs.writeFileSync(execTranscript, JSON.stringify({ type: "session_meta", payload: {
    id: "exec-session", cwd: "D:/dev/worktree-agent", originator: "codex_exec", source: "exec",
  } }) + "\n");
  r = runHook({ hook_event_name: "UserPromptSubmit", session_id: "s-exec", turn_id: "turn-exec",
    transcript_path: execTranscript, cwd: "D:/dev/worktree-agent", prompt: "<objective>" });
  check("T7 codex_exec receives no blocker context", r.status === 0 && r.stdout === "{}", `${r.stdout} ${r.stderr}`);
  fs.appendFileSync(execTranscript, JSON.stringify({ type: "response_item", payload: {
    type: "function_call", name: "request_user_input", call_id: "call-exec", arguments: "{}",
    internal_chat_message_metadata_passthrough: { turn_id: "turn-exec" },
  } }) + "\n");
  await new Promise((resolve) => setTimeout(resolve, 400));
  check("T7 codex_exec starts no request_user_input watcher", pendingEntries().length === 0);
  fs.appendFileSync(execTranscript, JSON.stringify({ type: "response_item", payload: {
    type: "function_call_output", call_id: "call-exec", output: "{}",
    internal_chat_message_metadata_passthrough: { turn_id: "turn-exec" },
  } }) + "\n" + JSON.stringify({ type: "event_msg", payload: {
    type: "task_complete", turn_id: "turn-exec",
  } }) + "\n");
  await new Promise((resolve) => setTimeout(resolve, 100));
  clearPending();
  r = runHook({ hook_event_name: "Stop", session_id: "s-exec", turn_id: "turn-exec",
    transcript_path: execTranscript, cwd: "D:/dev/worktree-agent",
    last_assistant_message: "Какую задачу выполнить?\nЖду вашего ответа." });
  check("T7 codex_exec text blocker stays silent", r.status === 0 && r.stdout === "{}"
    && pendingEntries().length === 0, `${r.stdout} ${r.stderr}`);

  clearPending();
  const sourceExecTranscript = path.join(sessions, "rollout-source-exec.jsonl");
  fs.writeFileSync(sourceExecTranscript, JSON.stringify({ type: "session_meta", payload: {
    id: "source-exec-session", cwd: "D:/dev/background-agent", originator: "automation", source: "exec",
  } }) + "\n");
  runHook({ hook_event_name: "Stop", session_id: "s-source-exec", turn_id: "turn-source-exec",
    transcript_path: sourceExecTranscript, cwd: "D:/dev/background-agent",
    last_assistant_message: "Need a task.\nWaiting for your answer." });
  check("T7 source=exec independently stays silent", pendingEntries().length === 0);

  clearPending();
  const interactiveTranscript = path.join(sessions, "rollout-interactive.jsonl");
  fs.writeFileSync(interactiveTranscript, JSON.stringify({ type: "session_meta", payload: {
    id: "interactive-session", cwd: "D:/dev/interactive", originator: "Codex CLI", source: "cli",
  } }) + "\n");
  runHook({ hook_event_name: "Stop", session_id: "s-interactive", turn_id: "turn-interactive",
    transcript_path: interactiveTranscript, cwd: "D:/dev/interactive",
    last_assistant_message: "Need your decision.\nWaiting for your answer." });
  rows = pendingEntries();
  check("T7 interactive CLI blocker still alerts", rows.length === 1
    && rows[0].value.attention_source === "text_contract", JSON.stringify(rows));
