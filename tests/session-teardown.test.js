...
(async () => {
  ...
  console.log("T17: attention rows survive Stop/PostToolUse for their dedicated owner");
  resetPending();
  const p17 = writePending("attention.json", { session_id: "s1", kind: "attention", hook_pid: 0 });
  check("T17 Stop exits 0", runTeardown(stop("s1")) === 0);
  check("T17 attention survives Stop", fs.existsSync(p17));
  check("T17 PostToolUse exits 0", runTeardown(postToolUse("s1", "Bash", { command: "echo x" })) === 0);
  check("T17 attention survives PostToolUse", fs.existsSync(p17));
