...
async function main() {
  ...
  // mid-value (the live regression that motivated this).
  {
    ...
    // multi-line + overlong: short collapses to one capped line; full keeps the
    // newlines (for the expand affordance) and carries past the 200 cap.
    await run("Bash", { command: "echo first\necho second\n" + "x".repeat(250) });
    const byTool = {}, byFull = {};
    for (const f of pendingFiles(dir)) {
      const j = JSON.parse(fs.readFileSync(f, "utf8"));
      byTool[j.tool_name] = j.tool_input_short;
      byFull[j.tool_name] = j.tool_input_full;
    }
    ...
    check("T13 short field is one line (newlines collapsed)", !/\n/.test(byTool.Bash || ""), `got=${JSON.stringify(byTool.Bash)}`);
    check("T13 tool_input_full preserves newlines", /\n/.test(byFull.Bash || ""), `got=${JSON.stringify(byFull.Bash)}`);
    check("T13 tool_input_full not truncated at 200 (carries the tail)", (byFull.Bash || "").length > 200, `len=${(byFull.Bash || "").length}`);
  }
  ...
  // T16: per-agent kill switch — ~/.press-1-off-claude present → hook exits 0
  // empty and writes no pending (press-1 out of the way → Claude's native prompt).
  {
    const dir = freshTemp("t16");
    fs.writeFileSync(path.join(dir, ".press-1-off-claude"), "");
    const h = runHookAsync(PAYLOAD, { ...hostEnv("panel"), USERPROFILE: dir }, dir);
    const code = await h.waitExit(5000);
    check("T16 off-flag → exit 0", code === 0, `code=${code}`);
    check("T16 off-flag → no stdout (native prompt)", h.stdout().trim() === "");
    check("T16 off-flag → no pending written", pendingFiles(dir).length === 0,
      `pending=${pendingFiles(dir).length}`);
  }

  // T17: the pending carries tool_key — the identity session-teardown.js matches
  // against the tool that just completed (PostToolUse), which is what drops the
  // card after a NATIVE answer. Liveness alone cannot: the current Claude Code
  // leaves the losing hook polling (live incident 2026-08-03). The digest is
  // pinned to the same literal as session-teardown.test.js T10 — the two files
  // hold independent copies of the formula and must never drift apart.
  {
    const dir = freshTemp("t17");
    const cmd = { command: 'ssh root@example "git fetch origin" 2>&1 | tail -8' };
    const h = runHookAsync({ ...PAYLOAD, tool_name: "Bash", tool_input: cmd }, hostEnv("panel"), dir);
    const pf = await waitFor(() => pendingFiles(dir)[0], 8000);
    check("T17 pending written", !!pf);
    if (pf) {
      const j = JSON.parse(fs.readFileSync(pf, "utf8"));
      check("T17 tool_key matches the pinned digest",
        j.tool_key === "4dd67e21f03c69d425d9269b70b99e5bf0c5a70b", `got=${j.tool_key}`);
    }
    h.child.kill();   // reap the blocked decision hook
  }
