...
; Isolate the new popup-display preference: tray callback tests must never touch
; the daily driver's real %USERPROFILE% file.
P1_POPUP_DISPLAY_PREFS_PATH := TESTROOT "\.press-1-popup-displays"
...
WritePending(id, proj, hostType, kind, optionsJson, waitUntil, pid, hookPid := -1, editorExe := "", agent := "claude", nativeControl := "false", timestamp := -1) {
    ...
    s := StrReplace(s, "__NOW__", timestamp)
...
; --- T9b: Codex attention has no hook beacon/decision channel. It must remain
; visible until its watcher/UserPromptSubmit owner removes the pending file. ---
WritePending("codex-attention", "proj-attn", "vscode-extension", "attention", "[]", 0, ownPid, 0, "Code.exe", "codex")
rows := ReadAllPrompts()
Check("T9b attention row present", rows.Length = 1, "got " rows.Length)
if rows.Length = 1 {
    Check("T9b attention is focus-only", rows[1].kind = "attention" && rows[1].decision_file = "")
    Check("T9b attention keeps Codex/editor identity", rows[1].agent = "codex" && rows[1].editor_exe = "Code.exe")
}
ClearPending()

; --- T9c: a hard-killed editor session may never emit SessionEnd. Once its
; attention row reaches the display backstop, the JSON must be deleted rather
; than becoming an invisible permanent orphan. Picker ownership is unchanged. ---
WritePending("codex-attention-expired", "proj-attn-exp", "vscode-terminal", "attention",
    "[]", 0, ownPid, 0, "Code.exe", "codex", "false", nowMs - STANDALONE_BACKSTOP_MS - 1)
rows := ReadAllPrompts()
Check("T9c expired attention row hidden", rows.Length = 0, "got " rows.Length)
Check("T9c expired attention pending deleted", !FileExist(PENDING_DIR "\codex-attention-expired.json"))
ClearPending()

; --- T9d: X/Esc acknowledges only attention notifications. A picker is still
; a live question owned by its hook and must survive the same popup dismissal. ---
WritePending("codex-attention-dismiss", "proj-attn-dismiss", "vscode-terminal", "attention",
    "[]", 0, ownPid, 0, "Code.exe", "codex")
WritePending("picker-dismiss-control", "proj-picker-dismiss", "vscode-terminal", "picker",
    "[]", 0, ownPid, ownPid, "Code.exe", "codex")
PromptPopup.promptData := ReadAllPrompts()
PromptPopup.visible := true
PromptPopup.DismissByUser()
Check("T9d dismiss deletes attention pending", !FileExist(PENDING_DIR "\codex-attention-dismiss.json"))
Check("T9d dismiss preserves picker pending", FileExist(PENDING_DIR "\picker-dismiss-control.json"))
PromptPopup.dismissedSignature := ""
...
; editor_exe carried for FindEditorCandidates.
...
; --- T36b: editor-terminal attention can run from a GSD worktree whose cwd
; basename is absent from the VS Code workspace title. The captured editor exe
; is enough to focus only when it owns exactly one window; ambiguity stays safe.
Check("T36b one preferred editor window → fallback", ResolveEditorFocusFallback([42], [42, 77]) = 42)
Check("T36b two preferred windows → ambiguous", ResolveEditorFocusFallback([42, 43], [42, 43]) = 0)
Check("T36b no preferred, one editor overall → fallback", ResolveEditorFocusFallback([], [77]) = 77)
Check("T36b no preferred, multiple editors overall → ambiguous", ResolveEditorFocusFallback([], [77, 88]) = 0)
...
WriteProxyPending("17780-0", "17780", "0", "pwsh -Command Set-Content", "D:/work/sample-project",
...
if rows.Length = 1 {
    r := rows[1]
    Check("T37 project_name = basename(cwd)", r.project_name = "sample-project", r.project_name)
...
Check("T48 release tooltip pinned to v8.1.1",
    InStr(prodAhk, 'A_IconTip := "press-1 v8.1.1"') > 0)
...
; --- T51: editor-terminal Focus must use the full activation path so a
; minimized VS Code/Cursor/Devin window can be restored. Extension panels keep
; the gentle path that prevents the historical multi-window focus war; unknown
; hosts fail conservatively into the same gentle policy. ---
Check("T51 editor terminal uses full activation", !P1_EditorFocusGentle("vscode-terminal"))
Check("T51 extension panel remains gentle", P1_EditorFocusGentle("vscode-extension"))
Check("T51 unknown editor host remains gentle", P1_EditorFocusGentle("mystery"))

; --- T52: duplicate editor titles are candidates to CYCLE, not a random first
; match. The selection order is numeric-HWND-stable because activating a window
; changes WinGetList's z-order; a z-order-based cycle can otherwise bounce
; between two windows and never reach the third. ---
cands := ResolveEditorFocusCandidates([43, 42], [43, 42, 99], [77], [77, 88])
Check("T52 duplicate preferred title matches preserved",
    cands.Length = 2 && cands[1] = 43 && cands[2] = 42)
cands := ResolveEditorFocusCandidates([], [42], [77, 88], [77, 88])
Check("T52 unique preferred exe fallback preserved", cands.Length = 1 && cands[1] = 42)
cands := ResolveEditorFocusCandidates([], [42, 43], [77], [77])
Check("T52 ambiguous nonmatching preferred windows still fail closed", cands.Length = 0)
cands := ResolveEditorFocusCandidates([], [], [77, 88], [77, 88])
Check("T52 cross-editor duplicate title matches preserved",
    cands.Length = 2 && cands[1] = 77 && cands[2] = 88)
cands := ResolveEditorFocusCandidates([], [], [], [77])
Check("T52 unique all-editor fallback preserved", cands.Length = 1 && cands[1] = 77)

cycleState := Map()
choice := P1_SelectEditorFocusCandidate([30, 10, 20], "prompt-a", cycleState)
Check("T52 cycle starts from stable smallest hwnd",
    choice.hwnd = 10 && choice.index = 1 && choice.total = 3)
choice := P1_SelectEditorFocusCandidate([20, 30, 10], "prompt-a", cycleState)
Check("T52 z-order change does not disturb second candidate", choice.hwnd = 20 && choice.index = 2)
choice := P1_SelectEditorFocusCandidate([30, 20, 10], "prompt-a", cycleState)
Check("T52 cycle reaches third candidate", choice.hwnd = 30 && choice.index = 3)
choice := P1_SelectEditorFocusCandidate([20, 10, 30], "prompt-a", cycleState)
Check("T52 cycle wraps", choice.hwnd = 10 && choice.index = 1)
choice := P1_SelectEditorFocusCandidate([50, 40], "prompt-a", cycleState)
Check("T52 changed candidate set resets cycle", choice.hwnd = 40 && choice.index = 1 && choice.total = 2)
choice := P1_SelectEditorFocusCandidate([30, 10, 20], "prompt-b", cycleState)
Check("T52 independent prompt starts its own cycle", choice.hwnd = 10 && choice.index = 1)

; --- T53: focus outline surrounds the target without changing its rectangle.
; Four strips leave the center click-through and work for snapped/maximized
; windows because the target itself is never moved or resized. ---
rects := P1_FocusOutlineRects(10, 20, 100, 80, 5)
Check("T53 outline has four strips", rects.Length = 4)
Check("T53 top strip", rects[1].x = 10 && rects[1].y = 20 && rects[1].w = 100 && rects[1].h = 5)
Check("T53 bottom strip", rects[2].x = 10 && rects[2].y = 95 && rects[2].w = 100 && rects[2].h = 5)
Check("T53 side strips preserve inner height",
    rects[3].x = 10 && rects[3].y = 25 && rects[3].w = 5 && rects[3].h = 70
    && rects[4].x = 105 && rects[4].y = 25 && rects[4].w = 5 && rects[4].h = 70)

; --- T54: visible focus bounds must stay on the selected monitor even when
; GetWindowRect includes Electron's invisible resize border. This is the real
; three-monitor failure: a maximized left window starts at x=-2568, while the
; visible monitor starts at x=-2560. ---
clipped := P1_IntersectFocusRect(
    {x:-2568, y:-8, w:2576, h:1416},
    {x:-2560, y:0, w:2560, h:1400})
Check("T54 maximized negative-coordinate rect clips to visible monitor",
    clipped && clipped.x = -2560 && clipped.y = 0 && clipped.w = 2560 && clipped.h = 1400)
clippedRects := P1_FocusOutlineRects(clipped.x, clipped.y, clipped.w, clipped.h, 5)
Check("T54 all outline strips remain on left monitor",
    clippedRects[1].x >= -2560 && clippedRects[1].y >= 0
    && clippedRects[2].y + clippedRects[2].h <= 1400
    && clippedRects[4].x + clippedRects[4].w <= 0)
Check("T54 disjoint window and monitor fail closed",
    !P1_IntersectFocusRect({x:10, y:10, w:20, h:20}, {x:100, y:100, w:50, h:50}))

P1_SetFocusMonitorDiagnostic(101, 101)
Check("T54 stable monitor diagnostic", P1_LAST_FOCUS_DIAGNOSTIC = "Last focus monitor: stable")
P1_SetFocusMonitorDiagnostic(101, 202)
Check("T54 changed monitor diagnostic", P1_LAST_FOCUS_DIAGNOSTIC = "Last focus monitor: CHANGED")

; Dynamic wrappers keep the RED phase runnable before the production helpers
; exist: missing functions become explicit assertion failures instead of aborting
; the whole harness before it can write the summary.
CallParsePopupDisplayPrefs(raw, &found) {
    global P1_ParsePopupDisplayPrefs
    found := IsSet(P1_ParsePopupDisplayPrefs)
    if !found
        return []
    try return P1_ParsePopupDisplayPrefs(raw)
    catch
        return []
}
CallSerializePopupDisplayPrefs(items, &found) {
    global P1_SerializePopupDisplayPrefs
    found := IsSet(P1_SerializePopupDisplayPrefs)
    if !found
        return ""
    try return P1_SerializePopupDisplayPrefs(items)
    catch
        return ""
}
CallResolvePopupTargets(catalog, prefs, &found) {
    global P1_ResolvePopupTargets
    found := IsSet(P1_ResolvePopupTargets)
    if !found
        return []
    try return P1_ResolvePopupTargets(catalog, prefs)
    catch
        return []
}
CallPopupScale(dpi, workWidth, &found) {
    global P1_PopupScale
    found := IsSet(P1_PopupScale)
    if !found
        return 0
    try return P1_PopupScale(dpi, workWidth)
    catch
        return 0
}
CallCardInstanceKey(promptKey, displayKey, &found) {
    global P1_CardInstanceKey
    found := IsSet(P1_CardInstanceKey)
    if !found
        return ""
    try return P1_CardInstanceKey(promptKey, displayKey)
    catch
        return ""
}
CallGetDisplayCatalog(&found) {
    global P1_GetDisplayCatalog
    found := IsSet(P1_GetDisplayCatalog)
    if !found
        return []
    try return P1_GetDisplayCatalog()
    catch
        return []
}
CallRefreshDisplayCatalog(rebuildMenu, &found) {
    global P1_RefreshDisplayCatalog
    found := IsSet(P1_RefreshDisplayCatalog)
    if !found
        return []
    try return P1_RefreshDisplayCatalog(rebuildMenu)
    catch
        return []
}
CallIsWorkAreaChange(wParam, &found) {
    global P1_IsWorkAreaChange
    found := IsSet(P1_IsWorkAreaChange)
    if !found
        return false
    try return P1_IsWorkAreaChange(wParam)
    catch
        return false
}

; --- T55: popup display preferences are a tiny fail-safe line protocol. No
; file/invalid content means Windows primary; valid rows preserve a stable
; physical display id plus a sanitized last-known label for disconnected UI.
prefs := CallParsePopupDisplayPrefs("v1`nmonitor-a`tAOC Left`nmonitor-b`tAOC Right`n", &prefHelperFound)
Check("T55 display preference parser exists", prefHelperFound)
Check("T55 valid display preferences parsed",
    prefs.Length = 2 && prefs[1].id = "monitor-a" && prefs[2].label = "AOC Right")
invalidPrefs := CallParsePopupDisplayPrefs("v2`nmonitor-a`tAOC Left`n", &prefHelperFound)
Check("T55 unknown preference version fails to primary", invalidPrefs.Length = 0)
dupPrefs := CallParsePopupDisplayPrefs("v1`nmonitor-a`tFirst`nmonitor-a`tLast`n", &prefHelperFound)
Check("T55 duplicate display ids collapse deterministically",
    dupPrefs.Length = 1 && dupPrefs[1].label = "Last")
serialized := CallSerializePopupDisplayPrefs([
    {id:"monitor-a", label:"AOC`tLeft`nInjected"},
    {id:"monitor-b", label:"AOC Right"}], &serializeHelperFound)
Check("T55 display preference serializer exists", serializeHelperFound)
Check("T55 serializer strips protocol-breaking label characters",
    serialized = "v1`nmonitor-a`tAOC Left Injected`nmonitor-b`tAOC Right`n", serialized)

; --- T56: default follows the Windows primary property, never enumeration
; index 1. Explicit selections resolve by stable id across index/position reorder.
catalog := [
    {id:"left", index:1, primary:false},
    {id:"right", index:2, primary:false},
    {id:"centre", index:3, primary:true}]
targets := CallResolvePopupTargets(catalog, [], &resolveHelperFound)
Check("T56 target resolver exists", resolveHelperFound)
Check("T56 no preference selects actual primary, not monitor 1",
    targets.Length = 1 && targets[1].id = "centre" && targets[1].index = 3)
reordered := [
    {id:"centre", index:1, primary:true},
    {id:"right", index:3, primary:false},
    {id:"left", index:2, primary:false}]
targets := CallResolvePopupTargets(reordered,
    [{id:"left", label:"Left"}, {id:"right", label:"Right"}], &resolveHelperFound)
Check("T56 explicit physical selection survives enumeration reorder",
    targets.Length = 2 && targets[1].id = "right" && targets[2].id = "left")

; --- T57: a partially connected explicit set renders only on its connected
; members. Primary is a temporary fallback only when every saved display is gone;
; the saved preference itself remains untouched for reconnect.
targets := CallResolvePopupTargets(catalog,
    [{id:"left", label:"Left"}, {id:"missing", label:"Dock"}], &resolveHelperFound)
Check("T57 partial selection does not add surprise primary",
    targets.Length = 1 && targets[1].id = "left")
targets := CallResolvePopupTargets(catalog,
    [{id:"missing-a", label:"Dock A"}, {id:"missing-b", label:"Dock B"}], &resolveHelperFound)
Check("T57 all missing selections temporarily fall back to primary",
    targets.Length = 1 && targets[1].id = "centre")

; --- T58: popup size is defined in 96-DPI logical pixels. Resolution alone no
; longer makes the card larger; Windows scaling does. Only a too-small work area
; may shrink it enough to fit.
scale96 := CallPopupScale(96, 2560, &scaleHelperFound)
scale144 := CallPopupScale(144, 3840, &scaleHelperFound)
scaleWide := CallPopupScale(96, 5120, &scaleHelperFound)
scaleNarrow := CallPopupScale(96, 500, &scaleHelperFound)
Check("T58 popup DPI scale helper exists", scaleHelperFound)
Check("T58 100 percent is one logical scale", Abs(scale96 - 1.0) < 0.001, scale96)
Check("T58 150 percent is one-and-a-half physical scale", Abs(scale144 - 1.5) < 0.001, scale144)
Check("T58 ultrawide resolution alone does not enlarge popup", Abs(scaleWide - 1.0) < 0.001, scaleWide)
Check("T58 too-small work area only shrinks to fit", scaleNarrow > 0 && scaleNarrow < 1.0, scaleNarrow)

; --- T59: multi-display reconciliation needs a composite instance identity.
; Prompt identity remains shared so selection/expand/click behavior stays global.
ik1 := CallCardInstanceKey("prompt-1", "display-a", &instanceHelperFound)
ik2 := CallCardInstanceKey("prompt-1", "display-b", &instanceHelperFound)
ik3 := CallCardInstanceKey("prompt-2", "display-a", &instanceHelperFound)
Check("T59 card instance helper exists", instanceHelperFound)
Check("T59 same prompt on different displays has distinct instance keys",
    ik1 != "" && ik1 != ik2)
Check("T59 different prompts on same display have distinct instance keys", ik1 != ik3)

; Runtime catalog smoke is topology-agnostic: whatever Windows currently exposes,
; the helper must return the same count, exactly one actual primary, and unique ids.
displayCatalog := CallGetDisplayCatalog(&catalogHelperFound)
Check("T59 display catalog helper exists", catalogHelperFound)
Check("T59 display catalog mirrors active monitor count",
    displayCatalog.Length = MonitorGetCount(), "catalog=" displayCatalog.Length " monitors=" MonitorGetCount())
primaryCount := 0, uniqueDisplayIds := Map(), catalogIdsValid := true
for display in displayCatalog {
    if display.primary
        primaryCount++
    if display.id = "" || uniqueDisplayIds.Has(display.id)
        catalogIdsValid := false
    else
        uniqueDisplayIds[display.id] := true
}
Check("T59 display catalog has exactly one Windows primary", primaryCount = 1, primaryCount)
Check("T59 display catalog ids are nonempty and unique", catalogIdsValid)

; --- T60: real tray callback persistence. Selecting one connected display writes
; its stable id atomically; selecting Primary removes the file and restores the
; dynamic default. This also catches callback binding/init regressions.
trayBuildOk := true, trayBuildError := ""
try P1_RebuildDisplayMenu()
catch as e {
    trayBuildOk := false
    trayBuildError := e.Message " line=" e.Line
}
Check("T60 popup display submenu builds without callback errors", trayBuildOk, trayBuildError)
if trayBuildOk && P1_DISPLAY_CATALOG.Length > 0 {
    firstLabel := P1_DisplayLabel(P1_DISPLAY_CATALOG[1], 1)
    try P1_TogglePopupDisplay(firstLabel, 0, P1_DISPLAY_MENU)
    catch
        trayBuildOk := false
    storedPrefs := P1_ReadPopupDisplayPrefs(P1_POPUP_DISPLAY_PREFS_PATH)
    Check("T60 display click persists stable physical id",
        trayBuildOk && storedPrefs.Length = 1
        && storedPrefs[1].id = P1_DISPLAY_CATALOG[1].id)
    try P1_SelectPrimaryDisplay()
    catch
        trayBuildOk := false
    Check("T60 Primary removes explicit preference file",
        trayBuildOk && !FileExist(P1_POPUP_DISPLAY_PREFS_PATH)
        && P1_POPUP_DISPLAY_PREFS.Length = 0)
}

; --- T61: WM_DISPLAYCHANGE may arrive while g.Show pumps messages inside a
; reconcile. Reload must defer instead of freeing card resources still owned by
; that stack frame; the outer reconcile consumes both pending flags afterwards.
PromptPopup.visible := true
PromptPopup._reconciling := true
PromptPopup._reconcilePending := false
PromptPopup._displayReloadPending := false
PromptPopup.ReloadDisplays()
Check("T61 display reload defers while reconcile owns cards",
    PromptPopup._displayReloadPending && PromptPopup._reconcilePending)
PromptPopup.visible := false
PromptPopup._reconciling := false
PromptPopup._reconcilePending := false
PromptPopup._displayReloadPending := false

; --- T62: taskbar/work-area geometry is volatile even when the connected
; monitor topology is unchanged. A stale startup catalog must be replaced at
; every Show(), while WM_SETTINGCHANGE/SPI_SETWORKAREA reloads a popup that is
; already visible. This pins the v8.1 regression where the card was anchored to
; the full monitor bottom and Windows alternated between covering the footer and
; letting the popup cover the taskbar depending on topmost z-order.
P1_DISPLAY_CATALOG := [{id:"stale", index:999, primary:true,
    workLeft:0, workTop:0, workRight:1, workBottom:1}]
refreshedCatalog := CallRefreshDisplayCatalog(false, &refreshCatalogFound)
Check("T62 display catalog refresh helper exists", refreshCatalogFound)
Check("T62 explicit refresh replaces stale work-area geometry",
    refreshedCatalog.Length = MonitorGetCount()
    && P1_DISPLAY_CATALOG.Length = MonitorGetCount()
    && P1_DISPLAY_CATALOG[1].id != "stale")
Check("T62 SPI_SETWORKAREA classifier exists",
    CallIsWorkAreaChange(0x002F, &workAreaHelperFound) && workAreaHelperFound)
Check("T62 unrelated setting change does not rebuild popup",
    !CallIsWorkAreaChange(0x001F, &workAreaHelperFound))
Check("T62 Show refreshes catalog before card reconciliation",
    RegExMatch(prodAhk,
        "s)static Show\(prompts\) \{.*?P1_RefreshDisplayCatalog\(false\).*?this\._SyncCards\(\)"))
Check("T62 work-area message is registered",
    InStr(prodAhk, "OnMessage(0x001A, P1_OnWorkAreaChange)") > 0)
