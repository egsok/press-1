...
try {
    ...
    # The terminating-error renderer may wrap and color its message at the host
    # width. The explicit recovery line is stable and proves the same contract.
    Check 'T3 failure says restart succeeded' ($result.Output -match 'Resident AutoHotkey restarted after font update') "`n$($result.Output)"
    foreach ($proc in $resident) { Stop-TestProcess $proc.ProcessId }
    Stop-TestProcess $other.Id
} finally {
    Clear-TestRoot
