...
function runWithFakeCodex(dir, mode, useAbsoluteOverride = false) {
  ...
  const commands = [CMD_PERM, CMD_TD, CMD_TD];
  fs.writeFileSync(server, `
const fs = require("fs");
const readline = require("readline");
if (process.env.FAKE_CODEX_MODE === "immediate_exit") process.exit(7);
const commands = ${JSON.stringify(commands)};
const hashes = ["sha256:permission", "sha256:stop", "sha256:post"];
const eventNames = ["permission_request", "stop", "post_tool_use"];
let cfg = "";
...
const entries = commands.map((command, i) => ({
  ...
  key: ["fake:permission:0:0", "fake:stop:0:0", "fake:post:0:0"][i],
...
// T15: trust verification is strict: all three installed press-1 entries must
...
// the installer in the manual-action state.
{
  ...
  check("T15 duplicate keys do not impersonate three installed hooks",
...
// verification, process markers, and exit codes without touching real Codex.
{
  ...
  check("T18 all three current hashes written to config.toml",
    ["sha256:permission", "sha256:stop", "sha256:post"].every((hash) => cfg.includes(hash)), cfg);
}
