...
//     merge succeeded AND all required press-1 entries were verified trusted.
...
const GSD_STOP_WRAPPER = "codex-gsd-context-monitor.js";
const OUR_FILES = ["codex-permission-request.js", "session-teardown.js", "codex-attention.js",
  GSD_STOP_WRAPPER];
...
const WANTED = [
  ...
  { event: "UserPromptSubmit", file: "codex-attention.js",         timeout: 5,
    additionalContextLimit: 300 },
  { event: "Stop",             file: "codex-attention.js",         timeout: 5 },
  { event: "SessionEnd",       file: "codex-attention.js",         timeout: 3 },
];
...
function mergeHooksFile() {
  ...
  // GSD 1.10.0 registers its context monitor on Stop but intentionally writes
  // no stdout for that event. Codex 0.147 requires JSON from every successful
  // Stop hook and reports the otherwise harmless monitor as failed. Preserve
  // the monitor's side effects through our narrow wrapper and normalize only
  // its Stop output to `{}`. The wrapper filename retains "gsd-context-monitor"
  // so GSD's own presence check still recognizes the registration.
  const gsdWrapperCommand = cmdFor(GSD_STOP_WRAPPER);
  const stopGroups = root.hooks.Stop;
  if (Array.isArray(stopGroups)) {
    for (const group of stopGroups) {
      if (!group || !Array.isArray(group.hooks)) continue;
      for (const h of group.hooks) {
        if (!h || typeof h.command !== "string") continue;
        if (h.command.includes("gsd-context-monitor") && !h.command.includes(GSD_STOP_WRAPPER)) {
          h.command = gsdWrapperCommand;
          changed = true;
          console.log("[merge-codex-hooks] Stop: wrapped incompatible GSD context monitor output");
        }
      }
    }
  }
  ...
  for (const w of WANTED) {
    ...
    for (const group of groups) {
      ...
      for (const h of group.hooks) {
        ...
        if (w.additionalContextLimit !== undefined
          && h.additionalContextLimit !== w.additionalContextLimit) {
          h.additionalContextLimit = w.additionalContextLimit; action = "updated";
        }
    ...
    if (!found) {
      ...
      if (w.additionalContextLimit !== undefined)
        entry.additionalContextLimit = w.additionalContextLimit;
...
// the expected script so duplicate rows cannot impersonate the required slots.
const REQUIRED_TRUST_SLOTS = [
  ...
  { id: "user_prompt_submit", eventNames: ["user_prompt_submit", "userPromptSubmit"],  file: "codex-attention.js" },
  { id: "attention_stop",     eventNames: ["stop"],                                    file: "codex-attention.js" },
  { id: "session_end",        eventNames: ["session_end", "sessionEnd"],               file: "codex-attention.js" },
];
...
async function autoTrust() {
  ...
  // Trust every installed press-1 command, including the optional GSD Stop
  // compatibility wrapper; required-slot verification remains strict below.
  const seen = new Map();
