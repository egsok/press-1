...
## Works with OpenAI Codex too

If you also run the [Codex CLI](https://github.com/openai/codex), press-1 gives its command approvals the same treatment: the same popup, the same hotkeys, and a `CODEX` chip on the card so you always know which agent is asking. The installer sets this up automatically — a Codex flavor of the hook goes to `~\.codex\hooks\` and is registered in `~\.codex\hooks.json` with the same safe-merge approach as for Claude. Codex prompts in the editor terminal, Windows Terminal, and the Codex VS Code panel work out of the box (tested with codex-cli 0.144.x). One honest note: **Always allow** for Codex saves a command-prefix rule — for some complex commands Codex ignores it and simply asks again next time.

You can switch press-1's Codex handling on/off any time from the tray menu (**Active for → Codex**); Claude handling has the same toggle.

### The Codex panel, natively — experimental proxy channel (opt-in)

The Codex **panel** in VS Code normally rides the same hook channel. There's also a deeper, native integration for it: a small stdio proxy (`codex-mitm`) that sits between the extension and its bundled `codex app-server`, mirrors approval requests into the popup, and injects your hotkey answer back — the panel card resolves instantly, exactly as if you'd clicked it, including a proper server-side **Always allow**.

Enable / disable from the repo folder (needs Node.js; reload your VS Code windows after either):

```powershell
.\enable-codex-proxy.ps1     # builds the wrapper exe and points chatgpt.cliExecutable at it
.\disable-codex-proxy.ps1    # removes the setting — the panel goes back to the bundled Codex
```

This one is **experimental, off by default, use at your own risk** — the honest caveats:

- It rides on the extension's `chatgpt.cliExecutable` setting, which OpenAI marks **"DEVELOPMENT ONLY"**. An extension update may break it at any time; the wrapper is built to fail *loud*, and the disable script reverts everything (a write-once backup of your `settings.json` is kept).
- The wrapper sits in the panel's critical path. It's relay-first by design — if press-1's part of it breaks, frames still pass through and the panel keeps working, you only lose the popup feature. If the panel ever misbehaves anyway: disable script + reload window.
- **Deny** through the proxy aborts the agent's current turn (a hard stop). The panel's native Deny button declines *gracefully* and lets the agent continue — it goes through a channel the proxy can't reach. Prefer the native button when you want the agent to keep working with your refusal.
...
**Requirements:** Windows 10/11, Claude Code 2.1.x, and an editor — VS Code, Cursor, or Windsurf (for the in-editor scenarios). `winget` (built into current Windows) powers the automatic prerequisite install — without it, install the two prerequisites by hand and re-run. Codex CLI is optional — if you use it, the installer wires it up too.
...
- Does the same for Codex: hooks to `~\.codex\hooks\`, a safe merge into `~\.codex\hooks.json` (harmless if you don't use Codex).
...
| Codex prompts show no popup | check the entries in `~\.codex\hooks.json`, and the tray menu — **Active for → Codex** must be checked |
| Codex panel misbehaves after enabling the proxy | `.\disable-codex-proxy.ps1`, then reload VS Code windows — the panel returns to the bundled Codex |
