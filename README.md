...
AI coding agents like Claude Code and Codex stop and ask before they do anything consequential — *run this command? edit this file? delete that?* Codex can also stop for an ordinary choice — *which plan should I follow? is this verification approved?* The catch: the prompt appears wherever the agent happens to be running, which is rarely the window you're looking at. So the agent sits and waits, you lose time and flow hunting for the prompt — or worse, you never notice it and your agent is just idle.

**press-1 surfaces every waiting permission or user-choice checkpoint in one small always-on-top popup and plays a soft sound. Approve permissions with one global hotkey; when Codex needs an ordinary answer, jump straight to the right terminal or panel.** Every decision is still yours; press-1 just removes the window-hunting between you and the prompt.
...
- **Codex is waiting for a choice, not a permission** — Native `request_user_input` prompts and explicit workflow checkpoints can otherwise look like a finished turn. press-1 turns them into an attention card and takes you back to the waiting window without inventing or typing an answer.
...
- Permission buttons map to the real 2- or 3-option layout and are clickable. Needs-user cards are deliberately focus-only: **Focus terminal / Focus panel** takes you to Codex without guessing or typing an answer. If several editor windows have the same title, repeat Focus to cycle through them; the selected window briefly lights up.
...
Codex support has two independent jobs: routing permission approvals and warning when a turn is blocked on an ordinary user answer. Permission requests use the standard hook or the separate opt-in VS Code panel proxy; needs-user alerts use the standard installation's attention hooks.
...
### Needs-user alerts — the v8 headline

Codex does not expose a general-purpose interactive question UI on every surface. A workflow may fall back to a numbered text checkpoint, while native `request_user_input` is available only in supported modes. In both cases the visible result can simply stop and wait.

press-1 watches the current Codex turn locally and surfaces only two high-confidence states: an unmatched native `request_user_input`, or an explicit final blocker such as “Reply with the option number” / “Waiting for your answer.” Broader question-like prose is not allowed to trigger a popup. The card shows the question and focuses the most likely editor or terminal window; it never sends a digit or free-text answer.

If two editor windows have the same workspace title, repeated Focus cycles through the exact-title matches in a stable order and briefly outlines the chosen window. Without a unique or exact title match, press-1 fails safe and leaves you in control. An extension could target the exact internal terminal tab, but v8 deliberately avoids adding one just for this feature.
...
- Questions and option pickers are focus-only: press-1 never guesses an answer or types digits blindly when the target window's internal focus cannot be proven.
...
The permission hook is a blocking gate before the native prompt, so it first runs the exact same-turn reviewer check and otherwise uses a short decision phase followed by handoff. Separate attention hooks watch native question calls and explicit text blockers, then create focus-only cards. The opt-in `codex-mitm` replaces only the permission hook for the VS Code panel and mirrors post-review app-server requests.
...
| `codex-attention.js` | needs-user detection and lifecycle for Codex questions and explicit text checkpoints |
| `codex-gsd-context-monitor.js` | preserves GSD Stop side effects while normalizing output for Codex |
...
| Codex stopped on a question but no attention card appeared | Only native `request_user_input` and explicit final answer contracts trigger live alerts; broad questions stay silent to avoid false positives. Re-run `install.ps1`, reload the editor, and verify the attention hooks through `/hooks` |
