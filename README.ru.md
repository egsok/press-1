...
**Сразу узнавайте, когда AI-агенту нужны вы: отвечайте на разрешения глобально или переходите прямо к ждущему вопросу.**
...
AI-агенты вроде Claude Code и Codex останавливаются и спрашивают разрешение перед чем-то важным — *выполнить команду? изменить файл? удалить?* А Codex может остановиться и на обычном выборе — *по какому плану идти? проверка принята?* Загвоздка в том, что запрос появляется там, где работает агент, — а это редко то окно, на которое вы сейчас смотрите. В итоге агент стоит и ждёт, вы теряете время и фокус, разыскивая запрос, — а то и вовсе его не замечаете, и агент просто простаивает.

**press-1 показывает все ждущие разрешения и пользовательские чекпоинты в одном маленьком окне поверх остальных и проигрывает мягкий звук. На разрешение можно ответить глобальным хоткеем; если Codex ждёт обычный ответ, press-1 перенесёт прямо в нужный терминал или панель.** Решение всегда за вами; press-1 лишь убирает беготню по окнам между вами и запросом.
...
- **Codex ждёт выбор, а не разрешение** — Нативный `request_user_input` и явные workflow-чекпоинты иначе легко принять за завершившийся тёрн. press-1 превращает их в карточку внимания и возвращает в ждущее окно, ничего не придумывая и не печатая за вас.
...
- Кнопки разрешений повторяют реальную раскладку на 2 или 3 опции и кликабельны. Needs-user карточки намеренно только фокусируют: **Focus terminal / Focus panel** переносит к Codex, не угадывая и не печатая ответ. Если несколько окон редактора называются одинаково, повторный Focus циклически перебирает их и на мгновение подсвечивает выбранное.
...
Поддержка Codex решает две независимые задачи: маршрутизирует permission approvals и сообщает, когда тёрн заблокирован на обычном ответе пользователя. Разрешения идут через стандартный hook или отдельный opt-in proxy панели VS Code; needs-user уведомления — через attention hooks обычной установки.
...
### Needs-user уведомления — главная фича v8

Codex не на каждой поверхности показывает универсальный интерактивный вопрос. Workflow может откатиться к текстовому чекпоинту с вариантами, а нативный `request_user_input` доступен только в поддерживаемых режимах. В обоих случаях тёрн может просто остановиться и ждать.

press-1 локально следит за текущим тёрном Codex и показывает только два высокоточных состояния: нативный `request_user_input` без ответа или явный финальный блокер вроде «Ответьте номером» / «Жду вашего ответа». Более широкая вопросительная проза popup не вызывает. Карточка показывает вопрос и фокусирует наиболее вероятное окно редактора или терминала; цифру или текст ответа она никогда не отправляет.

Если два окна редактора имеют одинаковый workspace-title, повторный Focus циклически перебирает exact-title совпадения в стабильном порядке и кратко обводит выбранное окно. Без точного или однозначного совпадения press-1 отказывает безопасно. Расширение могло бы адресовать конкретную внутреннюю вкладку терминала, но в v8 мы намеренно не добавляем его только ради этой функции.
...
- Вопросы и выбор из списка работают только через фокус: press-1 не угадывает ответ и не печатает цифры вслепую в окно, чей внутренний фокус нельзя доказать.
...
Permission hook Codex — блокирующий гейт перед родным prompt: сначала он делает точную same-turn проверку reviewer, а иначе использует короткую decision-фазу и handoff. Отдельные attention hooks следят за нативными вопросами и явными текстовыми блокерами, создавая focus-only карточки. Opt-in `codex-mitm` заменяет только permission hook панели VS Code и зеркалит уже прошедшие reviewer app-server запросы.
...
| `codex-attention.js` | needs-user детектор и lifecycle для вопросов Codex и явных текстовых чекпоинтов |
| `codex-gsd-context-monitor.js` | сохраняет side effects GSD Stop и нормализует stdout для Codex |
...
| Codex остановился на вопросе, но карточки внимания нет | Live-уведомление дают только нативный `request_user_input` и явный финальный контракт ответа; широкие вопросы молчат ради защиты от ложных срабатываний. Повторите `install.ps1`, перезагрузите редактор и проверьте attention hooks через `/hooks` |
