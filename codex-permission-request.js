...
// Every Codex permission host rides the decision channel — there is no scrape
// or companion path for Codex (unlike Claude's history). classifyHost can only
// ever return a member of this set (its fallback is conhost, never "unknown"),
// so a misclassification can affect the badge but never drop delivery.
...
process.stdin.on("end", () => {
  try {
    ...
    // A Codex session holds ONE approval at a time (the hook is a blocking
    // pre-prompt gate), and interrupt/Esc in the TUI fires neither PostToolUse
    // nor Stop — an unanswered pending from an earlier attempt lingers until its
    // TTL. A new prompt in this session therefore means every previous
    // same-session pending is dead: clear them first, or retries stack identical
    // cards and the popup's FIFO-active row targets a corpse (live smoke
    // 2026-07-02). Claude-side precedent: the picker cleanup in
    // permission-request.js — narrower there because Claude permission prompts
    // can coexist via parallel tool calls; Codex ones can't.
    if (entry.session_id) {
      try {
        for (const f of fs.readdirSync(PENDING_DIR)) {
          ...
          try {
            ...
            if (prev.agent === "codex" && prev.session_id === entry.session_id)
              fs.unlinkSync(full);
          } catch {}
        }
      } catch {}
    ...
    if (word === "" && mode === "hybrid" && fs.existsSync(finalPath)) {
      ...
      if (word === "") {
        ...
        try {
          ...
          const tmp2 = finalPath + ".tmp";
          fs.writeFileSync(tmp2, JSON.stringify(phase2, null, 2), "utf8");
          fs.renameSync(tmp2, finalPath);
