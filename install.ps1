...
# Brand fonts (NNV popup typography): loaded process-private by the script
# (AddFontResourceEx FR_PRIVATE) — nothing is installed system-wide. Optional:
# a missing fonts dir just means the popup falls back to Segoe UI / Consolas.
if (Test-Path ".\fonts") {
    New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\scripts\fonts" | Out-Null
    Copy-Item ".\fonts\*" "$env:USERPROFILE\scripts\fonts\" -Force
}
# Codex CLI hook (scenario A/B/C for Codex users). Self-contained under ~\.codex\hooks
# so the Codex Stop/PostToolUse commands resolve there too.
...
# --- Merge hooks into ~\.codex\hooks.json (Codex CLI, wrapper schema + auto-trust) ---
# Part 1 (hooks.json) is load-bearing; Part 2 (auto-trust via `codex app-server`) is
# best-effort and prints a manual `/hooks` fallback if codex isn't on PATH.
if (Get-Command node -ErrorAction SilentlyContinue) {
