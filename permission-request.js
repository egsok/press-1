...
// Per-agent kill switch. The tray submenu writes/removes ~/.press-1-off-claude;
// when present, press-1 gets out of the way entirely for this agent (early exit
// before any pending/sound → the agent's native prompt shows, no popup).
const OFF_FLAG = process.env.USERPROFILE
  ? path.join(process.env.USERPROFILE, ".press-1-off-claude")
  : "";

// Stable identity of THIS tool call, carried in the pending so the PostToolUse
// teardown can recognise "the tool this card was asking about has now run" and
// drop the card — the completed tool is proof the user answered, whatever the
// hook's liveness says. Needed because the current Claude Code no longer kills
// the losing PermissionRequest hook: after a native answer the hook keeps
// polling, so hook_pid stays alive and liveness alone left the row on screen
// for the full 60-min window (live incident 2026-08-03).
//
// Keys are sorted recursively so a different property order in the PostToolUse
// payload still matches. session-teardown.js carries a byte-identical copy of
// this formula (a shared module would add a third deployed file that either
// hook could fail to require); both sides are pinned to the same digest by
// hook.test.js T17 and session-teardown.test.js T10.
function toolKey(name, input) {
  ...
  try {
    ...
    return "";  // unhashable input → no key → teardown falls back to liveness
  }
}
...
process.stdin.on("end", () => {
  try {
    ...
    // Per-agent kill switch (~/.press-1-off-claude): exit 0 empty = native prompt,
    ...
    let agentOff = false;
    try { agentOff = !!(OFF_FLAG && fs.existsSync(OFF_FLAG)); } catch {}
    if (agentOff) process.exit(0);
    ...
    const entry = {
      ...
      tool_key: toolKey(data.tool_name, ti),
